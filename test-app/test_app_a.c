...
int main()
{
    ...
    if(hailer_app_register(&msg_handle,  APP_ID_APP_A) != HAILER_SUCCESS)
    {
        ...
        if(ret > 0)
        {
            if (FD_ISSET(msg_handle.comm_fd, &read_fds))
            {
               ...
               if(ret > 0)
               {
                    ...
                    {
                        printf("App A Recieved msg_type=%d from Appid=%d RcvrIp=%s\n", msg_hdr.msg_type, msg_hdr.sndr_app_id, msg_hdr.rcvr_ip);
                    }
                    else
                    {
                    ...
                    if(TEST_COMM_BTWN_NODES)
                    {
                         ...
                         */
                        strncpy(&msg_hdr.rcvr_ip, "192.168.1.192", strlen("192.168.1.192") + 1);
                    }
